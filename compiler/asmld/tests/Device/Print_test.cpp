...
TEST(Print, levelError) {
	...
	// Ensure assert was returned due to bad message.
	std::string output = std::string("asmld.exe [ERROR] error msg\n");
	EXPECT_EQ(OsStdout_getOutput(), output);
	EXPECT_EQ(OsExit_hasRet(), false);
}
...
TEST(Print, withFile) {
	...
	// Ensure assert was returned due to bad message.
	std::string output = "asmld.exe [ERROR] (foobar.c) error msg\n";
	EXPECT_EQ(OsStdout_getOutput(), output);
}
...
TEST(Print, withFileLine) {
	...
	// Ensure assert was returned due to bad message.
	std::string output = "asmld.exe [ERROR] (foobar.c/42) error msg\n";
	EXPECT_EQ(OsStdout_getOutput(), output);
}