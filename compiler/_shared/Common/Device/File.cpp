...
std::string File::getName(void) {
	...
	if (s_isOpen == false) {Terminate_assert("getName() for no file");}
...
RetErr_e File::open(std::string const& name, FileOp_e const dir) {
	...
	if (s_isOpen == true) {Terminate_assert("open() without closing file");}
...
uint8_t File::peek(void) {
	...
	if ((s_isOpen == false) || (s_dir != FILE_OP_READ)) {
		Terminate_assert("peek() without open read file");
	}
	...
	if(retErr) {Terminate_assert("peek() failed OS call");}
...
uint8_t File::pop(void) {
	...
	if ((s_isOpen == false) || (s_dir != FILE_OP_READ)) {
		Terminate_assert("pop() without open read file");
	}
	...
	// Bad operation- something beyond program has happened.
	if(retErr) {Terminate_assert("pop() failed OS call");}
...
void File::write(uint8_t const byte) {
	...
	if ((s_isOpen == false) || (s_dir != FILE_OP_WRITE)) {
		Terminate_assert("write() without open write file");
	}
	...
	// Bad operation- something beyond program has happened.
	if(retErr) {Terminate_assert("write() failed OS call");}
}
